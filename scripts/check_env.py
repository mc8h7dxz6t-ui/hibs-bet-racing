...
def main() -> int:
    ...
    lines = [
        ...
        f"  Matchbook:     {_flag(h.matchbook)} (odds + steam + routing)",
        f"  Betfair:       {'OFF (Phase C)' if not h.betfair_enabled else _flag(h.betfair_configured)}",
        f"  Routing:       {', '.join(h.execution_venues)}  dry-run={'yes' if not os.environ.get('HIBS_EXECUTION_LIVE') else 'NO'}",
