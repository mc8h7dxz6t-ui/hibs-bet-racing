...
chmod +x "${DAILY}" "${WEEKLY}" "${SCRIPT_DIR}/_lib.sh"

# 06:00 UK — cron uses machine local time; adjust TZ if server not in Europe/London.
DAILY_CRON="0 6 * * * ${DAILY} >> ${ROOT}/logs/cron_daily.log 2>&1"
WEEKLY_CRON="30 5 * * 0 ${WEEKLY} >> ${ROOT}/logs/cron_weekly.log 2>&1"

mkdir -p "${ROOT}/logs"

EXISTING="$(crontab -l 2>/dev/null || true)"
FILTERED="$(printf '%s\n' "${EXISTING}" | grep -v 'hibs-racing/scripts/daily_refresh.sh' | grep -v 'hibs-racing/scripts/weekly_retrain.sh' | sed '/^$/d' || true)"
