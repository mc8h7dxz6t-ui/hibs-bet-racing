...
- **Product pills in the racing UI:** `⚽ Football`, `📈 Trading`, `📊 Lines` point to the separate **hibs-bet football** product (not in this repo). By default Football → `/` and Trading/Lines → local paths that **404**. Link them by setting `HIBS_FOOTBALL_BASE_URL` / `HIBS_TRADING_STATUS_URL` / `HIBS_LINE_TRADER_URL`.
