...
| **Tenant migrate** | ~15% — enterprise services | **Do not SKU** | Clear enterprise pain; **no product code** today. Sell as **design-partner SOW** (procurement offline) + license once you have cash. Not a GTM motion pre-revenue. |
...
| **mesh-limiter standalone** | ~90% already in Proxy-Risk | **Feature, not SKU** | Rate bucket + Z-score + idempotency = Proxy-Risk. Sell as **Proxy-Risk Enterprise tier** (procurement offline). Never separate price list. |
...
New idea
   ...
   │
   ├─ Services-only (migrate, integrate)?  → SOW after first procurement offline)
