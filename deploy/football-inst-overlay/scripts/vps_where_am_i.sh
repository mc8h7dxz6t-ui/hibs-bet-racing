...
case "${role}" in
  main)
    ...
    echo ""
    echo "1) Sync overlay from public GitHub (no Mac scp):"
    echo "   curl -fsSL https://raw.githubusercontent.com/mc8h7dxz6t-ui/hibs-bet-racing/main/deploy/football-inst-overlay/deploy/vps-bootstrap-from-github.sh | \\"
    echo "     sudo HIBS_OVERLAY_REF=main FVE_REMOTE_HOST=${FVE_IP} bash"
    echo ""
    echo "   # or if scripts already on disk:"
    echo "   sudo HIBS_OVERLAY_REF=main bash ${BET}/deploy/vps-sync-overlay-from-github.sh"
    echo ""
    echo "2) Sync overlay from local racing tree:"
